...
from text_normalization import normalize_text, reference_tokens
...
def post_processing(
    ...
    for key, ref, hyp in results:
        new_ref = reference_tokens(" ".join(ref), language)
        new_hyp = reference_tokens(" ".join(hyp), language)
...
def decode_one_batch(
    ...
    if params.decoding_method == "fast_beam_search":
        ...
        hyp_tokens = fast_beam_search_nbest_oracle(
            ...
            num_paths=params.num_paths,
            ref_texts=sp.encode(
                texts_to_sp_inputs(supervisions["text"], params.language)
            ),
            nbest_scale=params.nbest_scale,
        )
        for hyp in sp.decode(hyp_tokens):
    ...
    for batch_idx, batch in enumerate(dl):
        ...
        for name, hyps in hyps_dict.items():
            ...
            for cut_id, hyp_words, ref_text in zip(cut_ids, hyps, texts):
                ref_words = reference_tokens(ref_text, params.language)
...
def save_results(
    ...
    for key, results in results_dict.items():
        ...
        results = post_processing(params.language, results)
    ...
    if "modified_beam_search" in params.decoding_method:
        if os.path.exists(params.context_file):
            ...
            if contexts:
                ...
                context_graph.build(
                    sp.encode(texts_to_sp_inputs(contexts, params.language))
                )
            else:
                context_graph = None
        else:
