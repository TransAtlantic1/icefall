...
- Acoustic feature: F5-TTS-compatible mel
- Feature dim for training: 100

`prepare_data.sh` is only a compatibility wrapper that forwards directly to `prepare.sh`.
...
4. Stage 3: offline resample recordings to 24 kHz FLAC cache
5. Stage 4: normalize text and build raw cuts
6. Stage 5: compute dev/test F5 mel features
7. Stage 6: split train raw cuts
8. Stage 7: compute train F5 mel features by split
...
The feature extractor used by `local/compute_fbank_emilia.py` is `local/f5tts_mel_extractor.py`, and the training feature dim is fixed at 100 in `zipformer/train.py`.
...
Why `--feature-num-workers 0`: this avoids Docker `/dev/shm` failures during batched feature extraction.
...
If you keep default `enable_musan=false`, no extra train-time flag is required because this recipe also defaults to `--enable-musan false` in its datamodule.

## 7. Expected artifacts

Core outputs:
- `data/manifests/<lang>/emilia_<lang>_recordings_{train,dev,test}.jsonl.gz`
- `data/manifests_resampled/<lang>/24000/...`
...
- zh: `zipformer/exp-zh-24k`
- en: `zipformer/exp-en-24k`
...
```text
zipformer/exp-<lang>-24k/tensorboard
```
...
python zipformer/decode.py \
  ...
  --epoch 30 \
  --avg 15 \
  --exp-dir "zipformer/exp-${LANG}-24k" \
  --max-duration 600 \
...
Expected feature dimension is `100`.
...
- Stage 3 writes cached 24 kHz FLAC files and updated manifests. Later feature extraction reads the cached audio through Lhotse, not through a manual WAV conversion step.
- The F5-style extractor can internally resample to 24 kHz, but this recipe is designed to use offline 24 kHz cache for scalability on larger datasets.
