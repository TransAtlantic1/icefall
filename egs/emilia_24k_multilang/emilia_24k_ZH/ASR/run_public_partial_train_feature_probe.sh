...
if [ -z "$source_artifact_root" ]; then
  source_artifact_root="${public_root%/}/emilia/fc71e07/icefall_emilia_${language}_24k"
fi

if [ -z "$probe_artifact_root" ]; then
  probe_artifact_root="${public_root%/}/emilia/fc71e07/icefall_emilia_${language}_24k_partial_train_probe"
fi
...
env CUDA_VISIBLE_DEVICES="" bash "${SCRIPT_DIR}/prepare.sh" \
  --language "$language" \
  --artifact-root "$probe_artifact_root" \
  --recording-num-splits "$recording_num_splits" \
  --feature-num-splits "$feature_num_splits" \
