...
from text_normalization import tokenize_zh_text
...
def normalize_emilia_args(args: argparse.Namespace) -> argparse.Namespace:
    ...
    args.lang_dir = Path(
        defaults["lang_dir"] if getattr(args, "lang_dir", None) in (None, "") else args.lang_dir
    )
    args.exp_dir = Path(
    ...
    if hasattr(args, "tokens"):
        args.tokens = str(
            defaults["tokens"]
            if getattr(args, "tokens", None) in (None, "")
            else args.tokens
        )
...
def texts_to_sp_inputs(texts, language: str):
    language = validate_language(language)
    if isinstance(texts, str):
        return tokenize_zh_text(texts) if language == "zh" else texts
    if language == "zh":
        return [tokenize_zh_text(text) for text in texts]
    return texts


def is_sentencepiece_tokenizer(tokenizer: Any) -> bool:
    return isinstance(tokenizer, spm.SentencePieceProcessor)


def load_tokenizer(
    lang_dir: Union[str, Path], bpe_model: Optional[Union[str, Path]] = None
):
    lang_dir = Path(lang_dir)
    if (lang_dir / "lang_type").exists():
        return Tokenizer.load(lang_dir)
    if (lang_dir / "bpe.model").exists():
        return Tokenizer.load(lang_dir, lang_type="bpe")
    if bpe_model not in (None, ""):
        tokenizer = spm.SentencePieceProcessor()
        tokenizer.load(str(bpe_model))
        return tokenizer
    raise FileNotFoundError(
        f"Could not determine tokenizer type from {lang_dir} and no --bpe-model was provided."
    )


def texts_to_tokenizer_inputs(texts, language: str, tokenizer: Any):
    if is_sentencepiece_tokenizer(tokenizer):
        return texts_to_sp_inputs(texts, language)
    return texts


def encode_texts(tokenizer: Any, texts, language: str):
    return tokenizer.encode(
        texts_to_tokenizer_inputs(texts, language, tokenizer), out_type=int
    )


def decode_token_ids(tokenizer: Any, token_ids: List[List[int]]) -> List[str]:
...
def decode_piece_sequence(tokenizer: Any, pieces: List[str]) -> str:
    if is_sentencepiece_tokenizer(tokenizer):
        return tokenizer.decode_pieces(pieces)
...
def log_wandb_metrics(
    ...
    wandb_run.log(
        {prefix + key: value for key, value in metrics.norm_items()},
        step=step,
    )


def log_wandb_scalars(
    wandb_run: Optional[Any], scalars: Dict[str, Any], step: Optional[int] = None
) -> None:
    if wandb_run is None:
        return
...
def setup_wandb(params: AttributeDict, rank: int) -> Optional[Any]:
    if not params.use_wandb or rank != 0:
        return None

    if not params.wandb_project:
        raise ValueError("--wandb-project must be provided when --use-wandb is true")

    try:
        import wandb
    except ImportError as exc:
        raise ImportError(
            "wandb is not installed. Install it or disable --use-wandb."
        ) from exc

    run_id_path = Path(params.exp_dir) / WANDB_RUN_ID_FILENAME
    run_id = params.wandb_run_id.strip()
    ...
    run = wandb.init(
        ...
        group=params.wandb_group or None,
        name=params.wandb_run_name or None,
        tags=parse_wandb_tags(params.wandb_tags) or None,
        job_type="train",
        config={
            **WANDB_RECIPE_CONFIG,
            ...
            "enable_musan": params.enable_musan,
            "exp_dir": str(params.exp_dir),
        },
        id=run_id or None,
        resume=params.wandb_resume,
    )
...
def get_parser():
    ...
    parser.add_argument(
        ...
        type=str,
        default="",
        help="Resume a specific Weights & Biases run id.",
    )
    parser.add_argument(
        "--wandb-resume",
        type=str,
        default="allow",
        help="Weights & Biases resume policy.",
    )
...
def compute_loss(
    ...
    y = encode_texts(tokenizer, batch["supervisions"]["text"], params.language)
    y = k2.RaggedTensor(y)
...
def run(rank, world_size, args):
    ...
    wandb_run = setup_wandb(params, rank)
    ...
    if wandb_run is not None:
        wandb_run.config.update({"num_model_params": num_param}, allow_val_change=True)
    ...
    valid_cuts = emilia.dev_cuts()
    valid_cuts = valid_cuts.filter(remove_short_utt)
    valid_dl = emilia.valid_dataloaders(
        valid_cuts,
        world_size=world_size,
        rank=rank,
    )
    ...
    if tb_writer is not None:
        tb_writer.close()
    if wandb_run is not None:
        wandb_run.summary["train/best_train_loss"] = params.best_train_loss
        wandb_run.summary["train/best_valid_loss"] = params.best_valid_loss
...
def display_and_save_batch(
    ...
    y = encode_texts(tokenizer, supervisions["text"], params.language)
