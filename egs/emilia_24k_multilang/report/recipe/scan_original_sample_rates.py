...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        type=Path,
        default=None,
        help="Existing JSON result file to merge scanned languages into.",
    )
...
def sort_language_results(language_results: Sequence[Dict[str, object]]) -> List[Dict[str, object]]:
    return sorted(language_results, key=lambda item: str(item["language"]))


def compute_total_result(
    language_results: Sequence[Dict[str, object]], elapsed_sec: Optional[float] = None
) -> Dict[str, object]:
    ...
    if elapsed_sec is None:
        elapsed_sec = sum(float(language_result["elapsed_sec"]) for language_result in language_results)
    ...
    return {
        ...
        "files_per_sec": round(total_files / elapsed_sec, 1) if elapsed_sec else 0.0,
    }


def merge_results(
    existing_result: Dict[str, object], new_language_results: Sequence[Dict[str, object]], config: ScanConfig
) -> Dict[str, object]:
    existing_languages = {
        language_result["language"]: language_result
        for language_result in existing_result.get("languages", [])
    }
    for language_result in new_language_results:
        existing_languages[language_result["language"]] = language_result

    merged_language_results = sort_language_results(list(existing_languages.values()))
    total_elapsed = sum(float(language_result["elapsed_sec"]) for language_result in merged_language_results)
    merged_result = {
        "generated_at_utc": datetime.now(timezone.utc).strftime("%Y-%m-%d %H:%M:%S UTC"),
        "config": {
            "dataset_root": str(config.dataset_root),
            "languages": [language_result["language"] for language_result in merged_language_results],
            "excluded_languages": sorted(
                set(discover_languages(config.dataset_root, []))
                - {str(language_result["language"]) for language_result in merged_language_results}
            ),
        ...
        "languages": merged_language_results,
        "total": compute_total_result(merged_language_results, total_elapsed),
    }
...
def build_markdown_report(results: Dict[str, object]) -> str:
    ...
    lines: List[str] = []
    lines.append("# Emilia 24k 原始采样率报告")
    lines.append("")
    lines.append(f"- 生成时间：`{generated_at}`")
    lines.append(f"- 数据集根路径：`{config['dataset_root']}`")
    lines.append(f"- 扫描语言：`{', '.join(config['languages'])}`")
    lines.append(f"- 排除语言：`{', '.join(excluded_languages) or '-'}`")
    lines.append(
        "- 扫描方法：直接读取原始 MP3 文件头，跳过 ID3 tag 后，解析第一个合法的 MPEG 音频帧头，得到源音频采样率。"
    )
    lines.append(
        f"- 扫描配置：`workers={config['workers']}`, `chunk_size={config['chunk_size']}`, "
    ...
    lines.append("## 汇总")
    lines.append("")
    lines.append("| 语言 | Batch 数 | 文件数 | 采样率分布 | 混合 Batch 数 | 坏文件数 | 扫描速度 |")
    lines.append("|---|---:|---:|---|---:|---:|---:|")
    ...
    for language_result in languages:
        ...
        lines.append("")
        lines.append(f"- 已扫描文件数：`{format_int(language_result['files'])}`")
        lines.append(f"- Batch 数：`{format_int(language_result['batch_count'])}`")
        lines.append(
            f"- 采样率分布：`{format_rate_counts(language_result['sample_rate_counts'])}`"
        )
        lines.append(
            f"- Batch 状态：`单一={format_int(language_result['uniform_batches'])}`, "
            f"`混合={format_int(language_result['mixed_batches'])}`, "
            f"`异常={format_int(language_result['bad_batches'])}`"
        )
        if language_result["sample_rate_examples"]:
            ...
            lines.append(f"- 示例文件：{sample_examples}")
        if language_result["bad_examples"]:
            bad_examples = ", ".join(f"`{path}`" for path in language_result["bad_examples"][:5])
            lines.append(f"- 坏文件示例：{bad_examples}")
        lines.append("")
        lines.append("| Batch | 文件数 | 采样率分布 | 坏文件数 | 状态 |")
        lines.append("|---|---:|---|---:|---|")
        for batch_result in language_result["batches"]:
            lines.append(
                ...
                f"{localize_status(str(batch_result['status']))} |"
            )
        lines.append("")
...
def main() -> None:
    ...
    new_language_results = [scan_language(config, language) for language in config.languages]

    if args.merge_json is not None:
        existing_result = json.loads(args.merge_json.read_text(encoding="utf-8"))
        result = merge_results(existing_result, new_language_results, config)
    else:
        language_results = sort_language_results(new_language_results)
        result = {
            "generated_at_utc": generated_at,
            "config": {
                "dataset_root": str(dataset_root),
                "languages": [language_result["language"] for language_result in language_results],
            ...
            "total": compute_total_result(language_results, time.time() - started),
        }
        ...
        json.dumps(
            {
                ...
                "total_files": result["total"]["files"],
                "elapsed_sec": result["total"]["elapsed_sec"],
                "files_per_sec": result["total"]["files_per_sec"],
            },
            ensure_ascii=False,
        ),
        flush=True,
