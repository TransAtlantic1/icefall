...
def get_parser():
    ...
    parser.add_argument(
        ...
        type=str2bool,
        default=False,
        help="Whether to log final decode metrics to Weights & Biases.",
    )
    parser.add_argument(
        "--wandb-project",
        type=str,
    ...
    parser.add_argument(
        ...
        type=str,
        default="",
        help="Existing Weights & Biases run id to resume for decode logging.",
    )
    parser.add_argument(
        "--wandb-resume",
        type=str,
...
def setup_wandb(params: AttributeDict) -> Optional[Any]:
...
def save_results(
    ...
    for key, val in test_set_wers:
        ...
        note = ""
    logging.info(s)
    return test_set_wers, errs_info
...
def main():
    ...
    for test_set, test_dl in zip(test_sets, test_dls):
        ...
        test_set_wers, summary_path = save_results(
            params=params,
            test_set_name=test_set,
            results_dict=results_dict,
        )
        if wandb_run is not None and test_set_wers:
            best_setting, best_wer = test_set_wers[0]
            wandb_run.summary[f"decode/{test_set}_best_setting"] = best_setting
            wandb_run.summary[f"decode/{test_set}_best_wer"] = best_wer
            wandb_run.summary[f"decode/{test_set}_summary_path"] = str(summary_path)
            wandb_run.log(
                {
                    f"decode/{test_set}_best_wer": best_wer,
                    f"decode/{test_set}_best_setting": best_setting,
                    f"decode/{test_set}_summary_path": str(summary_path),
                }
            )
