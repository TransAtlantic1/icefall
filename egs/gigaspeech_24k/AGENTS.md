...
This directory currently uses the shortened 24 kHz mainline under `ASR/`.
...
5. Keep all minimal real-data validation inputs and outputs under `experiments/`, not under recipe `data/`, `download/`, or public paths.
6. For the minimal 24 kHz validation subset, copy the needed source audio into `experiments/` and rewrite the subset manifests there so offline resampling stays experiment-local.

## Where Old Intermediate Ops Scripts Went

Use these locations:
- Current minimal validation scripts: `test/recipes/giga24k/`
...
- `run_smoke_train_offline.sh`: archived in `asr_op/giagspeech/backup/`, replaced by `test/recipes/giga24k/run_smoke_train.sh`
...
1. `test/recipes/giga24k/prepare_minimal_real_data.sh`
2. `test/recipes/giga24k/run_smoke_train.sh`
3. `test/recipes/giga24k/run_decode_export.sh`
4. `test/recipes/giga24k/validate_outputs.py`
