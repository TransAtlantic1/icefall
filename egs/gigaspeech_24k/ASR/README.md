...
The 24k experiment uses a custom Lhotse extractor that resamples 16kHz audio to 24kHz internally with `torchaudio`, so `torchaudio` must be available in the same environment as `icefall` and `lhotse`.
